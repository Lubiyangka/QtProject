...
TwoPlayers::TwoPlayers(QWidget *parent): QWidget(parent)
{
    //固定窗口
    ...
    label->setGeometry(1350,0,200,100);
    label->setText("玩家1:"
                   +QString::number(this->score(true))
                   +"\n"
                   +"玩家2:"
                   +QString::number(this->score(false))
                   );
...
//按键控制
void TwoPlayers::keyPressEvent(QKeyEvent *event){
    switch(event->key()){
    case Qt::Key_8:
...
//绘画函数
void TwoPlayers::paintEvent(QPaintEvent *event){
    //背景图片与右蛇
    QPixmap pix;
    pix.load(":/background/img/dessert.png");
...
//判断是否吃到食物
bool TwoPlayers::eaten(QRectF a){
...
//判断是否咬尾
bool TwoPlayers::dead(QList<QRectF> a){
...
//判断哪方咬尾
void TwoPlayers::bite_tie(QList<QRectF> a,QList<QRectF> b){
    for(int i = 1;i<a.length();++i)
...
//判断是否撞到障碍物
void TwoPlayers::crash(QRectF a,QRectF b){
...
//判断是否头相撞
void TwoPlayers::crash_head(QList<QRectF> a,QList<QRectF> b){
...
//
int TwoPlayers::score(bool isSanke){
    if(isSanke){
        return this->snake.length()-initialLength-1;
    }
    else{
        return this->snake1.length()-initialLength-1;
    }
}
